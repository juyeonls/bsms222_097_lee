---
title: "Human Genome Annotation tutorial"
output:
  html_document:
    df_print: paged
---
...
> ![Figure 1. Comparison of GENCODE and RefSeq gene annotation and the impact of reference geneset on variant effect prediction](https://media.springernature.com/full/springer-static/image/art%3A10.1186%2F1471-2164-16-S8-S2/MediaObjects/12864_2015_Article_7216_Fig1_HTML.jpg?as=webp)
...
```{r}
d1 = read_delim('gencode.v31.basic.annotation.gtf.gz', 
               delim='\t', skip = 5, 
               progress = F,
               col_names = cols)
```
...
```{r}
table(d1$feature_type)
```

```{r paged.print=FALSE}
d1 %>% group_by(feature_type) %>% count(feature_type)
```
...
```{r}
d1_1=filter(d1,feature_type=='gene')
# d1_1 = d[d1$feature_type == 'gene', ]
```
...
> ![Figure 2. Comparison of CCDS and Gencode](https://pbs.twimg.com/media/BiIoBo9IQAA2LQL?format=jpg&name=small)

Right. Then now we count how many genes annotated with HAVANA and ENSEMBL.
```{r paged.print=FALSE}
d1 %>% group_by(source) %>% count(source)
```
# 2.5 `do.call`
...
d2 <- d1 %>% filter(feature_type == 'transcript')
...
```{r}
#create a new colum that only contains the information from 'gene_id'
...
```{r paged.print=FALSE}
#Computes the number of transcripts per gene
ntxnpergene=d2%>%group_by(gene_id)%>%count()
#What is the mean number of transcripts per gene?
mean(ntxnpergene$n)
#What is the quantile (25%, 50%, 75%) for these numbers?
quantile(ntxnpergene$n,c(0.25,0.5,0.75))
#Which gene has the greatest number of transcript?
ntxnpergene[which.max(ntxnpergene$n),]
```
...
```{r paged.print=FALSE}
d2 %>% mutate(gene_type=as.character(do.call(rbind.data.frame, strsplit(d2$info, 'gene_type\\s+"'))[[2]]))%>%
 mutate(gene_type= as.character(do.call(rbind.data.frame,strsplit(gene_type, '\\"'))[[1]]))%>%
  group_by(gene_type,gene_id)%>%count()
#group_by(gene_type)%>%count(gene_id)%>%ungroup%>%group_by(gene_type)%>%count()
```
...
#not same with d2%>%group_by(chrom)%>%count()
...
```{r paged.print=FALSE}
d%>%
mutate(chr_group=case_when(seqnames%in%c('chrX','chrY')~"sex",
                              seqnames=='chrM'~"mitochondrial",
                              TRUE ~"autosomal"))%>%
filter(chr_group%in%c('autosomal','sex'))%>%
group_by(chr_group)%>%
summarize("0%"=quantile(width,0),"25%"=quantile(width,0.25),"50%"=quantile(width,0.5),"75%"=quantile(width,0.75),"100%"=quantile(width,1))
  #find simpler code
```
...
```{r paged.print=FALSE}
d%>%filter(type=='transcript',!is.na(transcript_support_level))%>%
  group_by(transcript_support_level)%>%count()
```
2. From the first question, please count the number of transcript for each TSL by gene biotype.
```{r paged.print=FALSE}
d%>%filter(type=="transcript",!is.na(transcript_support_level))%>%
  group_by(gene_type)%>%count(transcript_support_level)
```

3. From the first question, please count the number of transcript for each TSL by source.
```{r paged.print=FALSE}
d%>%filter(type=="transcript",!is.na(transcript_support_level))%>%
  group_by(source,transcript_support_level)%>%count()
```
...
```{r}
d_gene<-data.frame(gene_id=d$gene_id,gene_name=d$gene_name,hgnc_id=d$hgnc_id,gene_type=d$gene_type,chromosome=d$seqnames,start=d$start,end=d$end,strand=d$strand,isHgnc=ifelse(is.na(d$hgnc_id),"0","1"),isCCDS=ifelse(is.na(d$ccdsid),"0","1"))
```

2. Please count the number of hgnc by gene biotypes.
```{r paged.print=FALSE}
d_gene%>%group_by(gene_type)%>%filter(isHgnc=='1')%>%count()
#same with 
#d_gene%>%group_by(gene_type)%>%count(isHgnc=='1')
```
...
```{r paged.print=FALSE}
d_gene%>%mutate(level=d$level)%>%
  group_by(level)%>%
  filter(isHgnc=='1')%>%count()
```
...
```{r paged.print=FALSE}
d%>%filter(gene_type%in%c('protein_coding','lncRNA'))%>%
  group_by(gene_type)%>%
  summarize('0%'=quantile(width,0),'25%'=quantile(width,0.25),'50%'=quantile(width,0.5),'75%'=quantile(width,0.75),'100%'=quantile(width,1))
```
...
#mean
d%>%filter(type=='gene')%>%
  mutate(chrgroup=case_when(seqnames%in%c('chrX','chrY')~"sex",
                              seqnames=='chrM'~"mitochondrial",
                              TRUE ~"autosomal"))%>%
  filter(chrgroup%in%c('autosomal','sex'))%>%
  group_by(chrgroup)%>%
  count()%>%pull()%>%mean()
#median
d%>%filter(type=='gene')%>%
...
d%>%filter(type=='gene')%>%
  ...
  group_by(gene_type)%>%count(seqnames)
#there is no lncRNA gene type in chrM
```
