...
The formula for the sum of the series 1+2+\dots+n. What if we weren’t sure that was the right function? How could we check? Using what we learned about functions we can create one that computes the  
S_n:
```{r}
...
```{r}
ind<-murders$state
new_names<-ifelse(nchar(ind)>8,murders$abb,print("n/a"))
new_names
```
...
n computes the sum S_n = 1^2 + 2^2 + 3^2 + \dots n^2.Report the value of the sum when n=10.
```{r}
compute_s_n<- function(n){
  x<-1:n
  sum(x*x)
...
```{r}
s_n <- vector("numeric", 25) #(length=25)
for(n in 1:25){
  s_n[n] <- compute_s_n(n)
}
s_n
